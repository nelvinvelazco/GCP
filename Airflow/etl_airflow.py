...
def transformar_data(**kwargs):
    ...
    blobs = bucket.list_blobs(prefix=FOLDER_PATH+'test/')
    for blob in blobs:
...
def guardar_BigQuery(**kwargs):
    ...
    blobs = bucket2.list_blobs(prefix=FOLDER_PATH+'temp/')
    for blob in blobs:
        if blob.name.endswith('.csv'):            
            content = blob.download_as_string()
            #blob_csv= bucket2.blob(file_name)
            data= pd.read_csv(io.BytesIO(content), delimiter = ',', encoding = "utf-8")
